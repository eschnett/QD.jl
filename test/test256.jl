...
for n in 1:1000
    ...
    @test approxeq(Float256, 1, [], big(sqrt(abs(x))), sqrt(big(abs(x))))
    ...
    @test approxeq(Float256, 10, [1, x], big(asinh(x)), asinh(big(x)))
    @test approxeq(Float256, 10, [x + 2], big(acosh(x + 2)), acosh(big(x + 2)))
    @test approxeq(Float256, 10, [1, x], big(atanh(x)), atanh(big(x)))
end
